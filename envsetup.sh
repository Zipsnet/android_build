...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^icecold_") ; then
       ICECOLD_PRODUCT=$(echo -n $1 | sed -e 's/^icecold_//g')
    else
       ICECOLD_PRODUCT=
    fi
      export ICECOLD_PRODUCT
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${ICECOLD_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the ICECOLD model name
            lunch icecold_$target-userdebug
        fi
    fi
    return $?
}
...
function taco() {
    ...
    do
        ...
        if [ $? -eq 0 ]; then
            croot
            ./vendor/aokp/bot/build_device.sh icecold_$sauce-userdebug $sauce
        else
            echo "No such item in brunch menu. Try 'breakfast'"
        fi
    done
